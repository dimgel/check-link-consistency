...
namespace dimgel {
	...
	public:
		...
		void debug(const char* format, ...) noexcept __attribute__((format(printf, 2, 3))) {
			va_list args;
			va_start(args, format);
			impl(STDOUT_FILENO, pfxDEBG, colors.blue, format, args);

			// This satisfies cppcheck, but if impl() throws: https://stackoverflow.com/q/11645282/4247442
			va_end(args);
		}

		void info(const char* format, ...) noexcept __attribute__((format(printf, 2, 3))) {
			va_list args;
			va_start(args, format);
			impl(STDOUT_FILENO, pfxINFO, colors.green, format, args);
			va_end(args);
		}

		void exec(const char* format, ...) noexcept __attribute__((format(printf, 2, 3))) {
			va_list args;
			va_start(args, format);
			impl(STDERR_FILENO, pfxEXEC, colors.cyan, format, args);
			va_end(args);
		}

		void warn(const char* format, ...) noexcept __attribute__((format(printf, 2, 3))) {
			va_list args;
			va_start(args, format);
			impl(STDERR_FILENO, pfxWARN, colors.yellow, format, args);
			va_end(args);
		}

		void error(const char* format, ...) noexcept __attribute__((format(printf, 2, 3))) {
			va_list args;
			va_start(args, format);
			impl(STDERR_FILENO, pfxERR, colors.red, format, args);
			va_end(args);
		}
